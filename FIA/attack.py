...
tf.flags.DEFINE_float('beta', 0, 'parameter of weight')
...
tf.flags.DEFINE_string('layer_name', 'vgg_16/conv4/conv4_3/Relu', 'The layer to be attacked.')

tf.flags.DEFINE_string('input_dir', './dataset/images/', 'Input directory with images.')

tf.flags.DEFINE_string('output_dir', './adv/FIA+CLMA/', 'Output directory with images.')
...
"""parameter for DIM"""
tf.flags.DEFINE_integer('image_size', 224, 'size of each input images.') ### 224 299
...
# layer_name = vgg_16/conv3/conv3_3/Relu
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            for images,names,labels in utils.load_image(FLAGS.input_dir, FLAGS.image_size,FLAGS.batch_size):
                ...
                # obtain true label
                labels = np_utils.to_categorical(np.concatenate([labels,labels],axis=-1),num_classes)
                ...
                for i in range(num_iter):
                    ...
                    if i == 0 and 'FIA' in FLAGS.attack_method:
                        ...
                        # normalize both FIA and CLMA weights
                        weight_np = -normalize(weight_np, 2)
                        weight_torch = torch.load('weight/weight_tensor' + str(count) + '.pt',map_location='cpu')
                        weight_torch = weight_torch.cpu().numpy()
                        weight_torch = weight_torch.transpose((0, 2, 3, 1))
                        weight_torch = normalize(weight_torch, 2)
                        weight_np = weight_np + weight_torch * FLAGS.beta
